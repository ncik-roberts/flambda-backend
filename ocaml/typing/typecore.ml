...
type type_function_result =
  { function_ : type_expr * function_param list * function_body;
    ...
    newtypes: (string loc * Jane_asttypes.layout_annotation option) list;
...
and type_newtype
  ...
  let body, ety =
    type_newtype_gen loc env name layout_annot_opt (fun env ->
...
and type_n_ary_function
    ...
    *)
    ...
    | true ->
        ...
        let layout_var () = Layout.any ~why:Dummy_layout in
        let ty_function =
          List.fold_right
            (fun param rest_ty ->
              newty
                (Tarrow
                   ((param.fp_arg_label, mode_var (), mode_var ()),
                    newvar (layout_var ()), rest_ty, commu_ok)))
            params
            (match body with
            | Tfunction_body _ -> newvar (layout_var ())
            | Tfunction_cases _ ->
              newty
                (Tarrow
